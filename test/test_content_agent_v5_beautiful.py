#!/usr/bin/env python3
"""
Beautiful CLI Test for Content Agent V5
======================================

A visually rich terminal interface for testing Content Agent V5 with the mock strawman.
Features real-time progress, beautiful formatting, and comprehensive output.

Usage:
    python test_content_agent_v5_beautiful.py                # Default run
    python test_content_agent_v5_beautiful.py --pause        # Pause between slides
    python test_content_agent_v5_beautiful.py --export       # Save results
    python test_content_agent_v5_beautiful.py --quiet        # Minimal output
    python test_content_agent_v5_beautiful.py --no-icons     # Disable emojis
...
class BeautifulTestRunner:
    ...
    async def _process_slide_verbose(
            ...
            if not self.options.get('quiet'):
                ...
                for comp in planned_components:
                    ...
                    if comp.component_type == "text":
                        playbook_data = content_agent_v5.TEXT_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "analytics":
                        playbook_data = content_agent_v5.ANALYTICS_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "image":
                        playbook_data = content_agent_v5.IMAGE_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "diagram":
                        playbook_data = content_agent_v5.DIAGRAM_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "table":
                        playbook_data = content_agent_v5.TABLE_PLAYBOOK.get(comp.selected_playbook_key, {})
        ...
        for brief in strategic_briefs.briefs:
            if brief.component_type == "text":
                task = content_agent_v5.run_text_specialist_v4(
    ...
    def _export_results(self, manifests: List[ContentManifest], strawman: PresentationStrawman, processing_time: float):
        ...
        export_path = Path(__file__).parent / f"content_agent_v5_results_{timestamp}.json"
...
async def main():
    parser = argparse.ArgumentParser(
        description="Beautiful CLI test for Content Agent V5",
