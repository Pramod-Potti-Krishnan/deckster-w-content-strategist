#!/usr/bin/env python3
"""
Beautiful CLI Test for Content Agent V6
======================================

A visually rich terminal interface for testing Content Agent V6 with the mock strawman.
Features real-time progress, beautiful formatting, and comprehensive output.

Usage:
    python test_content_agent_v6_beautiful.py                # Default run
    python test_content_agent_v6_beautiful.py --pause        # Pause between slides
    python test_content_agent_v6_beautiful.py --export       # Save results
    python test_content_agent_v6_beautiful.py --quiet        # Minimal output
    python test_content_agent_v6_beautiful.py --no-icons     # Disable emojis
...
from src.agents.content_agent_v6 import ContentAgentV6, ContentManifest
from src.agents import content_agent_v6
...
[dim]Type: {slide.slide_type} | ID: {slide.slide_id}[/dim]
    ...
    def show_strategic_brief(self, brief):
        ...
        brief_tree = Tree(
            ...
            f"[bold cyan]{brief.component_type.upper()}[/bold cyan] - "
            f"[bold green]{playbook_display}[/bold green]"
        )
    ...
    def show_generated_content(self, comp_type: str, content: Any):
        ...
        if comp_type == "text" and hasattr(content, 'title'):
            ...
            if hasattr(content, 'content_blocks'):
                for block in content.content_blocks:
                    ...
                    if hasattr(block, 'content_html'):
                        ...
                        html = block.content_html.replace('<p>', '  ').replace('</p>', '\n')
            ...
            if hasattr(content, 'content_blocks'):
                for block in content.content_blocks:
                    if hasattr(block, 'content_html') and block.content_html:
                        lines.append(f"[yellow]{block.role}:[/yellow] [cyan]{block.content_html}[/cyan]")
            lines.append("")
            ...
            lines = [
                f"[bold green]{content.title}[/bold green]",
            ...
            lines = [
                f"[bold magenta]Archetype: {content.archetype}[/bold magenta]",
                f"[dim]Subject: {content.primary_subject}[/dim]",
                ""
            ]
            ...
            if hasattr(content, 'composition_notes'):
                ...
                lines.append(f"  {content.composition_notes}")
                lines.append("")
            ...
            if hasattr(content, 'alt_text'):
                ...
                lines.append(f"  {content.alt_text}")
            ...
            if hasattr(content, 'imagen_prompt') and content.imagen_prompt:
                ...
                for word in prompt_words:
                    ...
                    if len(' '.join(current_line)) > 70:
                        lines.append(f"  {' '.join(current_line[:-1])}")
                        current_line = [word]
                if current_line:
            ...
            lines = [
                f"[bold blue]Pattern: {content.pattern}[/bold blue]",
                f"[dim]Flow Direction: {content.flow_direction}[/dim]",
                ""
            ]
            ...
            if hasattr(content, 'summary_insight'):
                lines.append(f"[dim]{content.summary_insight}[/dim]")
                lines.append("")
            ...
            if hasattr(content, 'headers') and hasattr(content, 'rows'):
                ...
                lines.append(f"  Headers: {' | '.join(content.headers)}")
                lines.append(f"  {'-' * 50}")
            ...
            if hasattr(content, 'html_table') and content.html_table:
                ...
                if len(content.html_table) > 500:
                    html_preview += "..."
                lines.append(f"[cyan]{html_preview}[/cyan]")
...
class BeautifulTestRunner:
    ...
    async def _process_slide_verbose(
        ...
        components = await content_agent_v6.identify_required_components(slide)
        component_playbooks = await content_agent_v6.select_playbook_strategies(slide, components)
        ...
        for comp_type, playbook_key in component_playbooks.items():
            planned = content_agent_v6.PlannedComponent(
            ...
            if not self.options.get('quiet'):
                ...
                for comp in planned_components:
                    ...
                    if comp.component_type == "text":
                        playbook_data = content_agent_v6.TEXT_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "analytics":
                        playbook_data = content_agent_v6.ANALYTICS_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "image":
                        playbook_data = content_agent_v6.IMAGE_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "diagram":
                        playbook_data = content_agent_v6.DIAGRAM_PLAYBOOK.get(comp.selected_playbook_key, {})
                    elif comp.component_type == "table":
                        playbook_data = content_agent_v6.TABLE_PLAYBOOK.get(comp.selected_playbook_key, {})
        ...
        strategic_briefs = await content_agent_v6.run_strategic_briefing_agent_v2(
            planned_components,
            slide,
            theme,
        ...
        for brief in strategic_briefs.briefs:
            if brief.component_type == "text":
                task = content_agent_v6.run_text_specialist_v4(
                ...
                task = content_agent_v6.run_analytics_specialist_v4(brief, slide, theme)
                specialist_tasks.append(task)
                component_types.append("analytics")
            elif brief.component_type == "image":
                task = content_agent_v6.run_image_specialist_v4(brief, slide, theme)
                specialist_tasks.append(task)
                component_types.append("image")
            elif brief.component_type == "diagram":
                task = content_agent_v6.run_diagram_specialist_v4(brief, slide, theme)
                specialist_tasks.append(task)
                component_types.append("diagram")
            elif brief.component_type == "table":
                task = content_agent_v6.run_table_specialist_v4(brief, slide, theme)
                specialist_tasks.append(task)
        ...
        manifest = content_agent_v6.assemble_content_manifest_v4(
            slide,
            component_outputs,
            self.playbook_session
        )
    ...
    def _export_results(self, manifests: List[ContentManifest], strawman: PresentationStrawman, processing_time: float):
        ...
        export_path = Path(__file__).parent / f"content_agent_v6_results_{timestamp}.json"
...
async def main():
    parser = argparse.ArgumentParser(
        description="Beautiful CLI test for Content Agent V6",
