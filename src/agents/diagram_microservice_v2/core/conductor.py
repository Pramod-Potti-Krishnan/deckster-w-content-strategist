...
class DiagramConductor:
    ...
    async def _save_to_storage(self, request: DiagramRequest, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Saved diagram {diagram_id} to storage: {url}")
            
            return result
            
        except Exception as e:
            logger.error(f"Failed to save to storage: {e}")
            # Still return the result with inline content if storage fails
            result["content_delivery"] = "inline"
            result["diagram_id"] = str(uuid.uuid4())
            result["url"] = ""  # Empty URL when storage fails
            return result
