"""
Content Agent V7 - Content Generation Without Image Building
===========================================================

This module implements a streamlined content generation pipeline that:
- Generates all text content
- Creates image specifications and prompts (but not actual images)
- Generates table content
- Creates analytics specifications
- Creates diagram specifications

Based on V6 but with image generation moved to separate Image Build Agent.
...
Author: AI Assistant
Date: 2024
Version: 7.0 (Split Architecture)
"""
...
# ============================================================================
# CONTENT MANIFEST MODELS (V6 - Independent)
# ============================================================================

class VisualSpec(BaseModel):
    """
    Specification for any visual element (image, chart, diagram).
    Simplified for V6 - only fields actually used.
    """
    visual_type: str = Field(
        description="Type: 'image', 'chart', 'diagram', 'table', 'icon'"
    )
    description: str = Field(
        description="Detailed description or prompt for the visual"
    )
    placement_guidance: str = Field(
        default="primary",
        description="Placement guidance: 'primary', 'supporting', 'inline'"
    )
    style_notes: Dict[str, Any] = Field(
        default_factory=dict,
        description="Visual-specific metadata and styling information"
    )
...
class ContentManifest(BaseModel):
    ...
    V6 version with only the fields actually used.
    """
    slide_id: str
    slide_type: str
...
"Minimalist vector icon of a stethoscope, flat design style, centered composition on white background. Simple line art with 2-3 colors: medical blue and teal accents. Clean, modern, professional iconography suitable for healthcare presentations." """
    ...
    # Phase 2: Image generation moved to separate Image Build Agent
    # This function now returns the image specification only
    # The actual image can be generated by calling the Image Build Agent with result.data
    logger.info("Image specification created. Actual generation handled by Image Build Agent.")
    
    return result.data

# ============================================================================
# IMAGE GENERATION FUNCTIONS MOVED TO image_build_agent.py
# ============================================================================
# The following functions have been extracted to create a separate image service:
# - remove_white_background()
# - remove_background_advanced()
# - should_remove_background()
# - generate_image_with_imagen3()
#
# To generate images, use the Image Build Agent with the ImageContentV4 spec
# returned by run_image_specialist_v4()
...
# ============================================================================
# STAGE 4: ASSEMBLY - Moved to utils/content_assembly.py
# ============================================================================
# Assembly function has been moved to keep content generation and data formatting separate.
# Import from src.utils.content_assembly when needed.
...
async def process_single_slide(
...
) -> Union[ContentManifest, Dict[str, Any]]:
    """
    Process a single slide through the clean V7 pipeline.
    
    Args:
        slide: Slide to process
        theme: Theme definition
        deck_summary: Summary of the deck
        strawman: Presentation strawman
        completed_slides: Previously completed slides
        playbook_session: Playbook session
        return_raw: If True, return raw component outputs instead of assembled manifest
        
    Returns:
        Either ContentManifest (default) or Dict[str, Any] with raw outputs if return_raw=True
    """
    ...
    if return_raw:
        logger.info("Returning raw component outputs (assembly skipped)")
...
# ============================================================================
# CONTENT AGENT V7 CLASS
# ============================================================================

class ContentAgentV7:
    """
    Content Agent V7 - Content generation without image building.
    
    Generates all content specifications including image prompts,
    but delegates actual image generation to the Image Build Agent.
    """
    
    def __init__(self):
        """
        Initialize Content Agent V7.
        """
