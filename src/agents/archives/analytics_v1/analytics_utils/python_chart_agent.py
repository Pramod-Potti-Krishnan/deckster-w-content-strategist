...
# Using the simplified MCP implementation (validated 2024-08-11)
# The original complex implementation is archived but can be restored if needed
from .mcp_executor_simplified import get_simplified_executor, execute_chart_with_simplified_mcp as execute_chart_with_mcp
logger = logging.getLogger(__name__)

# FALLBACK OPTION (kept for emergency use only):
# To revert to the original complex implementation, uncomment below:
# from .archived_mcp_backup.original_implementation.mcp_integration import get_mcp_integration, execute_chart_with_mcp

# For compatibility with existing code that checks this flag
USE_SIMPLIFIED_MCP = True
...
class PythonChartAgent:
    ...
    def __init__(self, mcp_tool=None):
        ...
        """
        # Use simplified executor (validated and approved)
        self.mcp_integration = get_simplified_executor(mcp_tool)
        
        # FALLBACK OPTION (if ever needed):
        # To use original implementation, replace above line with:
    ...
    async def _execute_python_code(self, code: str) -> Optional[str]:
        ...
        try:
            # Simplified executor returns a dictionary with type and content
            result = await self.mcp_integration.execute_chart_code(code)
            if result.get("type") == "image":
                return result.get("content")
            else:
                # Code was returned instead of image (MCP not available)
                logger.debug(f"MCP returned code instead of image: {result.get('message')}")
                return None
