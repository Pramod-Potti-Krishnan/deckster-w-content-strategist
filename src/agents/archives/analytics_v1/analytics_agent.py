...
# Feature flag for playbook system
USE_PLAYBOOK_SYSTEM = os.getenv("USE_PLAYBOOK_SYSTEM", "false").lower() == "true"

if USE_PLAYBOOK_SYSTEM:
    # Use new playbook-based implementations
    logger = logging.getLogger(__name__)
    logger.info("[Analytics] Using PLAYBOOK-BASED conductor and synthesizer")
    from .analytics_utils.playbook_conductor import PlaybookConductor as AnalyticsConductor
    from .analytics_utils.playbook_synthesizer import PlaybookSynthesizer as DataSynthesizer
else:
    # Use original rule-based implementations
    from .analytics_utils.conductor import AnalyticsConductor
    from .analytics_utils.data_synthesizer import DataSynthesizer
    logger = logging.getLogger(__name__)
    logger.info("[Analytics] Using ORIGINAL rule-based conductor and synthesizer")
...
# Using simplified MCP executor (validated 2024-08-11)
from .analytics_utils.mcp_executor_simplified import get_simplified_executor as get_mcp_integration

# FALLBACK: To restore original implementation, replace above import with:
# from .analytics_utils.archived_mcp_backup.original_implementation.mcp_integration import get_mcp_integration
...
class AnalyticsAgent:
    ...
    async def generate_analytics(
        self,
        content,  # Can be str or AnalyticsRequest
        ...
        Args:
            content: Natural language description or AnalyticsRequest object
        ...
        """
        # Handle both string and AnalyticsRequest inputs
        if isinstance(content, AnalyticsRequest):
            request = content
            logger.info(f"Processing analytics request: {request.title}")
        else:
            logger.info(f"Processing analytics request: {str(content)[:100]}...")
